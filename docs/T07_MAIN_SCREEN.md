...
`workout_completions`, а frontend показывает тренировку на сегодня и ближайшую следующую, недельный прогресс, плеер/placeholder и
...
- заголовок «Сегодня», номер текущей недели и прогресс `X/7`;
- тренировку текущего дня и ближайшую следующую незавершённую тренировку этой недели;
- состояния `completed`, `available`, `locked`, а также обводку текущего дня;
- кнопку «Открыть полное расписание»: все семь дней и переключение недель находятся в Плане;
- HTML5 player для подписанного `video_url` и текст `Видео скоро будет доступно` для `null`;
- четыре fixed-вкладки: Сегодня, План, Прогресс и Тренер (`/chat`), с unread badge;
- верхнюю кнопку с инициалом профиля для Настроек, включая активное состояние на `/settings`.

Для профиля с `onboarding_status = base_lessons` тот же маршрут и tab bar доступны в режиме
знакомства. Сегодня и План показывают только разрешённые подпиской метаданные, а попытка
...
по history state;
пока completion сохраняется, вкладки временно получают `aria-disabled`, верхняя кнопка Настроек получает `disabled`, а системный Back
...
тесты фиксируют текущую и следующую тренировку, прогресс, текущий день, четыре нижние вкладки и верхнюю кнопку Настроек,
