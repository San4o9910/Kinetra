...
The first Node source build [timed out after 60 minutes with two parallel jobs](https://github.com/San4o9910/Kinetra/actions/runs/34442249626) while compiling V8. The corrected recipe uses four jobs with a 90-minute compile deadline and requires at least four available CPUs and 8 GiB effective memory capacity before starting. It records actual CPU/memory limits before compilation. This follows Node's [build prerequisites](https://github.com/nodejs/node/blob/v22.23.2/BUILDING.md#prerequisites); the public-repository Ubuntu runner has the [documented capacity](https://docs.github.com/en/actions/reference/runners/github-hosted-runners). These are CI build resources, separate from the application host. Authentication, runtime and scanner checks are unchanged.
...
This flag controls payment availability only. It does not create subscriptions, grant free beta access, change existing training entitlements or remove onboarding requirements. Accounts without an active subscription retain the existing training restriction. Production authentication still requires a valid `AUTH_TOKEN_DELIVERY_WEBHOOK_URL` and `AUTH_TOKEN_DELIVERY_WEBHOOK_SECRET`, HTTPS, secure refresh cookies and the existing database/TLS checks.
...
Omitting `PAYMENTS_ENABLED` preserves the previous `true` behavior for existing installations; only the exact strings `true` and `false` are accepted. A later payment rollout must separately configure and validate API credentials, exact HTTPS return URLs and the renewal job's own credentials, then restart the API with `PAYMENTS_ENABLED=true`. Validate enabled and disabled behavior before allowing traffic; renewal scheduling and real charges require their existing explicit authorization. To return to deferred mode, quiesce the renewal scheduler/process first, set both flags false, clear provider credentials and restart the API. The flag is startup configuration, so changes do not affect an already running process. Disabling incoming webhooks while provider payments are in flight requires a separate reconciliation decision; do not silently lose payment results.
