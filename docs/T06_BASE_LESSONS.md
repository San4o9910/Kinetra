...
T06 реализует обязательную подготовку перед первой тренировкой, не ограничивая знакомство с
остальным приложением. Пользователь со статусом `base_lessons` видит семь вводных уроков,
сохраняет прогресс просмотра и получает доступ к запуску тренировок после завершения как минимум
четырёх уроков.

## Маршрутизация

Канонический frontend-маршрут для профиля с `onboardingStatus: "base_lessons"` — `/`.
Пользователь видит основную оболочку Kinetra и может свободно переключаться между вкладками
Сегодня, План и Прогресс; Настройки открываются верхней кнопкой с инициалом. Маршрут `/base-lessons` остаётся доступной явной
страницей подготовки. На состояниях загрузки, ошибки и списка есть действие «Вернуться в
приложение»; из открытого плеера пользователь сначала возвращается к списку штатной кнопкой.

Источник истины для маршрута и разблокировки — ответы backend. Клиент не сохраняет статус
онбординга локально и не может самостоятельно запустить тренировку. После успешного завершения
этапа сервер меняет статус на `active`, а frontend остаётся на `/` в полном режиме приложения.

## Режим знакомства

Статус `base_lessons` разрешает навигацию по основной оболочке, но не ослабляет существующие
ограничения подписки и безопасности:

- без активной подписки программа и расписание сохраняют текущий paywall;
- с активной подпиской пользователь может видеть метаданные программы и расписания;
- workout `video_url` и `poster_url` для статуса `base_lessons` всегда равны `null`, даже если
  media опубликовано и подписка активна;
- попытка открыть тренировку показывает объяснение и предлагает перейти к базовым урокам либо
  вернуться к обзору приложения;
- `PUT /api/v1/program/complete-workout` до статуса `active` запрещён сервером;
- чат остаётся недоступным до статуса `active`.

Порог в четыре урока, серверный переход `base_lessons → active` и subscription/paywall правила
не меняются. Режим знакомства отделяет просмотр интерфейса от допуска к тренировкам, но не
предоставляет доступ к платным media или мутациям прогресса тренировок.
...
Отдельная кнопка «Вернуться в приложение» доступна независимо от количества завершённых уроков:
она возвращает на `/` в режиме знакомства и не меняет `onboardingStatus`.
...
- frontend unit: семь названий, три отрендеренных состояния карточки, динамическая CTA,
  placeholder, сериализация, ожидание final flush и возврат в приложение без изменения статуса;
- browser acceptance: список, placeholder, system Back, реальный десятисекундный PUT на 45%,
  ...
  последовательное обновление CTA, свободный возврат на `/` до порога, запрет запуска тренировки,
  переход в `active` после четырёх уроков и восстановление активного профиля после reload.
