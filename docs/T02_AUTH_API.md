...
## Ссылки из писем и интерфейс восстановления

На экране входа ссылка «Забыли пароль?» открывает форму запроса. Ответ не раскрывает,
зарегистрирован ли введённый email. Backend отдаёт нейтральный ответ и при сбое доставки;
его нельзя считать подтверждением того, что письмо действительно отправлено.

В режиме `webhook` доставляющий сервис получает JSON с полями `version`, `event`,
`recipient`, `token`, `expiresAt` и должен сформировать ссылку на точный HTTPS origin приложения:

- `password_reset` → `/auth/reset-password#token=<token>`;
- `email_verification` → `/auth/verify-email#token=<token>`.

Токен передаётся во fragment после `#`, а не в query: fragment не передаётся HTTP серверу.
Frontend удаляет его из адресной строки до отрисовки приложения и держит только в памяти,
без localStorage/sessionStorage. После перезагрузки очищенной страницы нужно снова открыть
исходную ссылку из письма. Сброс требует ввода нового пароля; подтверждение email требует
явного нажатия кнопки, поэтому открытие ссылки почтовым сканером не расходует токен.
Истёкшая, повреждённая или использованная ссылка не даёт доступ к аккаунту.

`AUTH_EMAIL_VERIFICATION_REQUIRED` сейчас по умолчанию `false`; добавление экрана не включает
обязательное подтверждение email. Производственный backend требует настроенную доставку:
`webhook` с действующим HTTPS обработчиком и его секретом или `smtp` с Яндекс/Gmail.
В режиме SMTP приложение формирует эти же ссылки самостоятельно и отправляет письмо
через защищённое соединение на фиксированный сервер выбранной почты, порт 465.
Адрес отправителя совпадает с SMTP username; принимается один email-получатель.
Пароли приложений, адрес получателя и токены не записываются в журналы ошибок.
Телефонный получатель не перенаправляется в почту и получает ошибку доставки.
Настройки: [Яндекс Почта для Kinetra](YANDEX_MAIL_SETUP.md).
SMTP пароль нельзя подставлять в `AUTH_TOKEN_DELIVERY_WEBHOOK_SECRET`: это разные способы
доставки. Наличие интерфейса и тесты с синтетическими ответами не подтверждают реальную доставку.
