...
Доставляющий сервис получает существующий webhook JSON с полями `version`, `event`,
...
обязательное подтверждение email. Производственный backend всё равно требует действующий
`AUTH_TOKEN_DELIVERY_WEBHOOK_URL` и соответствующий `AUTH_TOKEN_DELIVERY_WEBHOOK_SECRET`.
Это контракт собственного HTTPS обработчика, а не готовая интеграция SMTP: SMTP пароль
или произвольный API ключ нельзя подставлять вместо совместимого обработчика. Наличие
интерфейса и прохождение тестов с синтетическими ответами не подтверждают реальную доставку.
