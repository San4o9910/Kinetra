...
Экран сохраняет исходные четыре секции в прежнем порядке:
...
Ниже добавлены две секции:

5. «Путь на 12 недель»: нативный progress показывает `total_workouts / 84`, а шкала
   отмечает только `metrics.current_week`. По агрегату не выводятся завершения отдельных
   недель; `total_weeks_completed` показан отдельным фактом.
6. «Ваше самочувствие целиком»: SVG-радар последней заполненной недели, видимый список четырёх
   оценок и четыре графика динамики. Последняя запись выбирается по `program_week`, сравнение
   показывает разницу с предыдущей заполненной неделей без вывода о здоровье. Пустое состояние
   не получает выдуманных баллов; одна запись даёт радар, а линии появляются от двух записей.

Оценки — ответы самого пользователя, не медицинский или физиологический показатель.
Исходный основной график сохраняет собственные test hooks; дополнительные графики имеют
префиксы `trend-energy-chart`, `trend-sleep-chart`, `trend-mood-chart`, `trend-body_satisfaction-chart`.

Визуальная основа: угольный фон с тонким кольцом и диагональю, светлая бумажная тема,
лаймовое действие и сдержанный красно-коричневый акцент. CSS-токены находятся в `styles.css`.
Новых шрифтов, изображений и графических библиотек нет. Анимации появления, линии и радара
короткие, без бесконечного движения; `prefers-reduced-motion` их отключает.
