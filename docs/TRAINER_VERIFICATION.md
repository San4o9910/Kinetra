...
проходит строгую валидацию. На одну заявку допускается от 0 до 20 уникальных
URL. Поле `materials` можно не передавать: сервер использует пустой список.
Сайт и сертификаты по ссылке не обязательны. Владелец проверяет описание
подготовки и опыта, при необходимости запрашивает уточнения.
Переносы строк в описании поддерживаются. Права тренера выдаются только после
ручного одобрения; проверка email и запрет самостоятельного одобрения сохранены.
