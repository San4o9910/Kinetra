...
def main():
 ...
 api=folder=None
 try:
  assert sys.argv[1:]==['--upgrade-training-experience']
  ...
  request=pathlib.Path(os.environ['RUNNER_TEMP'],'kinetra-training-experience-request.json')
  info=request.lstat();assert not request.is_symlink() and info.st_uid==os.geteuid() and info.st_mode&0o777==0o600 and info.st_size<262144
  receipt=json.loads(request.read_bytes());assert receipt['control']==os.environ['GITHUB_SHA'] and receipt['run']==os.environ['GITHUB_RUN_ID']
  token=os.environ.pop('TIMEWEB_CLOUD_TOKEN','');assert token and not any(c.isspace() for c in token)
  deadline=time.monotonic()+2100;api=m.Api(token,m.SERVER_ID,deadline);token=''
  ...
  script=(ROOT/'training-experience-upgrade-host.py').read_text()
  result=subprocess.run(args+['/usr/bin/timeout --signal=TERM --kill-after=300s 1500s /usr/bin/python3 -B -c '+shlex.quote(script)],input=json.dumps(receipt['payload']).encode(),stdout=subprocess.PIPE,stderr=subprocess.DEVNULL,timeout=1850)
  assert len(result.stdout)<65536
  guest=json.loads(result.stdout);state['guest']=guest
  assert result.returncode==0 and guest['result']=='PASS_UPGRADED'
  state['result']='PASS_UPGRADED'
  ...
  pathlib.Path(os.environ['RUNNER_TEMP'],'kinetra-training-experience-upgrade.json').write_text(json.dumps(state,sort_keys=True)+'\n')
