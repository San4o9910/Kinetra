...
def main():
 ...
 pr=api('/pulls/22');assert pr['merged'] and pr['merge_commit_sha']==MERGED and pr['head']['sha']==APP
 ...
 raw=api('/actions/artifacts/10423615458/zip',archive=True);assert hashlib.sha256(raw).hexdigest()==digest
 with zipfile.ZipFile(io.BytesIO(raw)) as z:
  ...
  def member(name):
   ...
   return z.read(info)
  source=json.loads(member('source-gates.json'));assert source['app_commit']==APP and source['base_commit']==BASE and source['merge_commit']=='b993357ab2b3b157b82f977f2877f4f40b24120d'
  ...
  decision=module.evaluate(module.strict_json(member('upstream-media/production.json')),member('source-components.cdx.json'),BASE)
 ...
 path=pathlib.Path(os.environ['RUNNER_TEMP'],'kinetra-coaching-request.json');fd=os.open(path,os.O_WRONLY|os.O_CREAT|os.O_EXCL,0o600)
