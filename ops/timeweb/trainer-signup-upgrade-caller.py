...
def main():
 ...
 api=folder=None
 try:
  assert sys.argv[1:]==['--upgrade-trainer-signup']
  ...
  request=pathlib.Path(os.environ['RUNNER_TEMP'],'kinetra-trainer-signup-request.json')
  ...
  script=(ROOT/'trainer-signup-upgrade-host.py').read_text()
  ...
  pathlib.Path(os.environ['RUNNER_TEMP'],'kinetra-trainer-signup-upgrade.json').write_text(json.dumps(state,sort_keys=True)+'\n')
