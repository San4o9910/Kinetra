...
CLI: --resume-stopped-local-application --private-input /absolute/owner0600/input.json
...
def private_input(argv):
    require(len(argv) == 3 and argv[:2] == ["--resume-stopped-local-application", "--private-input"], "EXPLICIT_LOCAL_ACTIVATION_ARGUMENTS_REQUIRED")
    path = Path(argv[2])
...
def validate_start(value):
    ...
    if value['result'] == 'APPLICATION_LOCAL_ACCEPTED_ONLY':
        ...
        require({'/', '/health', '/ready', '/api/v1/me'} <= set(http) and any(p.startswith('/assets/') and p.endswith('.js') for p in http)
...
def main():
    ...
    try:
        ...
        child = subprocess.Popen(['/usr/bin/python3', '-B', str(folder / 'resume-stopped-application-host.py'),
            '--resume-exact-stopped-application', '--private-input', str(private)],
