#!/usr/bin/env node
// Anonymous acceptance on the approved host. Inspect login and registration without submission.
...
try{
  ...
  for(const [path,status] of [['/health',200],['/ready',404],['/api/v1/me',401],['/trainer/students',200],['/trainer/lessons',200],['/my-training',200],['/api/v1/training/students',401],['/api/v1/training/lessons',401],['/api/v1/training/mine',401]]){
  ...
  for(const [name,width,height,mobile] of [['desktop',1440,1000,false],['mobile',390,844,true]]){
    ...
    await evaluate("[...document.querySelectorAll('button')].find(b=>b.textContent.trim()==='Создать аккаунт').click()");
    for(let n=0;n<30;n++){
      if(await evaluate("Boolean(document.querySelector('[data-testid=register-screen]'))"))break;
      await pause(100);
    }
    await pause(300);
    const registration=await evaluate(`(()=>({
      rendered:Boolean(document.querySelector('[data-testid=register-screen]')),
      passwords:document.querySelectorAll('input[type=password]').length,
      email:document.querySelector('[data-testid=register-email]')?.type,
      minimum:document.querySelector('[data-testid=register-password]')?.minLength,
      confirmation:Boolean(document.querySelector('[data-testid=register-password-confirmation]')),
      phone:Boolean(document.querySelector('[data-testid=register-phone]')),
      roles:document.querySelectorAll('input[name=requested-role]').length,
      disabled:document.querySelector('[data-testid=register-submit]')?.disabled,
      overflow:document.documentElement.scrollWidth>innerWidth+1
    }))()`);
    assert.deepEqual(registration,{rendered:true,passwords:1,email:'email',minimum:6,confirmation:false,phone:false,roles:2,disabled:true,overflow:false});
    report.viewports.at(-1).simplifiedRegistrationRendered=true;
    const registrationShot=await send('Page.captureScreenshot',{format:'png',captureBeyondViewport:false});
    await writeFile(join(output,name+'-registration.png'),Buffer.from(registrationShot.data,'base64'));
    await evaluate("[...document.querySelectorAll('button')].find(b=>b.textContent.trim()==='Вернуться ко входу').click()");
    for(let n=0;n<30;n++){
      if(await evaluate("Boolean(document.querySelector('[data-testid=login-screen]'))"))break;
      await pause(100);
    }
  ...
  report.checks.push('simplified_registration_without_form_submission');
