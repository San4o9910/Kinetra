...
class StoppedContinuationTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.guest=load(folder/'resume-stopped-application-host.py')
        cls.auth=load(folder/'authenticate-stopped-containers.py')
    @classmethod
    ...
    def setUp(self):
        ...
        self.patches=[];self.patch('STAGE',self.root)
    ...
    def test_exact_scope_and_unchanged_acceptance(self):
        self.assertEqual(self.g.OWNER_APPROVAL,'1be0c7f18c7a9ee2d3835ce22cf7deefbd98e969')
        ...
        old=f(ROOT/'start-application-host.py');new=f(ROOT/'resume-stopped-application-host.py')
        for name in set(old)-{'main','activate','read_handoff'}:self.assertEqual(new[name],old[name],name)
        for path in ['start-application-host.py','resume-stopped-application-host.py']:
    ...
    def test_symlink_and_foreign_attempts_refused(self):
        for name in [self.g.RESUME_ATTEMPT,self.g.RESUME_RESULT,'application-assets-other.json','application-continuation-other.json','https-start-attempt.json']:
    ...
    def test_missing_or_changed_attempt_cannot_install_success(self):
        ...
        with self.assertRaisesRegex(self.g.Error,'EXACT_DURABLE_ASSET_ATTEMPT_REQUIRED'):
    ...
    def test_no_app_create_and_attempt_before_start(self):
        node=next(n for n in ast.parse((ROOT/'resume-stopped-application-host.py').read_text()).body if isinstance(n,ast.FunctionDef) and n.name=='activate')
    ...
    def test_wrapper_pins_and_explicit_entrypoint(self):
        wrapper=load(Path(self.files.name)/'resume-stopped-local-host.py')
        self.assertEqual(set(wrapper.public_helpers()),set(wrapper.PINS));self.assertIn('resume-stopped-application-host.py',wrapper.PINS)
        self.assertIn('--resume-exact-stopped-application',wrapper.GUEST_LAUNCHER)
    ...
    def test_fresh_actual_inspection_and_any_drift(self):
        value=json.loads((ROOT/'stopped-containers-inspection-20260914.json').read_text())
