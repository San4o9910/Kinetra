"""Install personal trainer programs, additive migration 016 and private host media.
...
S=pathlib.Path('/srv/kinetra-stage');APP='5f9fbea3ab2ed75f5da79d3fb542be4525b00aa3';BASE='c4f63e798b3bb98b97ea9eb7de73ba5c167a54c5'
OLD_IMAGES={'backend':'ghcr.io/san4o9910/kinetra-backend@sha256:b45a3bf54ec9ff4a7c2988147ae87554e95659cfa2b485ba0e45535887302e02','frontend':'ghcr.io/san4o9910/kinetra-frontend@sha256:252be3a21c0f89e93a7400b530d6c86f46be8305aee6e770c1f5c271054047c5'}
...
assert.ok(/^016_/.test(filename));await c.query('BEGIN');try{await c.query(text);await c.query('INSERT INTO schema_migrations(filename,checksum)VALUES($1,$2)',[filename,checksum]);await c.query('COMMIT')}catch(e){await c.query('ROLLBACK');throw e}}
...
def main():
 ...
 try:
  ...
  oldledger=ledger(pgid);expected=data['migrations'];require(len(expected)==16 and len(oldledger)==15 and all(expected.get(r['filename'])==r['checksum'] for r in oldledger),'MIGRATION_HISTORY_CHANGED')
  ...
  prior=S/'trainer-signup-release-c4f63e798b3b-35285117508'
  ...
  work=S/('trainer-workspace-release-'+APP[:12]+'-'+data['run']);work.mkdir(mode=0o700)
  ...
  media_parent=S/'media';media_parent.mkdir(mode=0o700,exist_ok=True)
  ...
  if not media.exists():media.mkdir(mode=0o700);os.chown(media,1000,1000)
  require(not media.is_symlink() and media.stat().st_uid==1000 and media.stat().st_gid==1000 and stat.S_IMODE(media.stat().st_mode)==0o700 and not list(media.iterdir()),'FIRST_MEDIA_DIRECTORY_INVALID')
  ...
  require(not (S/'source/deploy/compose.training-media.yml').exists(),'MEDIA_ALREADY_INSTALLED')
  ...
  media_check="""import assert from 'node:assert/strict';import {writeFile,unlink,stat} from 'node:fs/promises';import {randomUUID} from 'node:crypto';import {databasePool} from './apps/backend/dist/db/pool.js';import {TrainingService} from './apps/backend/dist/training/service.js';
...
const service=new TrainingService(databasePool,true);assert.deepEqual(await service.myTraining(randomUUID()),{trainer_name:null,student_id:null,plans:[]});await databasePool.end();console.log('LIVE_TRAINER_WORKSPACE=PASS');"""
  ...
  require(all(get(p)[0]==401 for p in ('/api/v1/training/students','/api/v1/training/mine','/api/v1/training/lessons','/api/v1/training/media/00000000-0000-4000-8000-000000000001?token=invalid')),'TRAINING_AUTH_BOUNDARY_FAILED')
  ...
  installed_config=S/'source/deploy/compose.training-media.yml'
  write(installed_config,media_compose)
  replace_private(S/'env/production.env',encoded(prod));replace_private(S/'env/api.env',encoded(api))
  state.update(result='PASS_UPGRADED',stage='COMPLETE',app=APP,images=data['images'],containers={k:c['Id'] for k,c in now.items()},migrations=len(expected),password_min_length=6,chat_enabled=True,private_trainer_media=True,trainer_workspace=True)
