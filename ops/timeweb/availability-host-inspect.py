...
def main():
 ...
 ids=raw.decode().split();assert len(ids)==3
 code,raw=run(['docker','inspect',*ids]);assert code==0
 ...
 assert all(c['Config']['Labels'].get('com.docker.compose.project')=='kinetra-production' for c in containers)
 db=None
 if selected['postgres']['State']['Running']:
