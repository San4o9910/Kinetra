...
def main():
 ...
 api=folder=None
 try:
  assert sys.argv[1:]==['--upgrade-coaching-release']
  ...
  request=pathlib.Path(os.environ['RUNNER_TEMP'],'kinetra-coaching-request.json')
  ...
  script=(ROOT/'coaching-upgrade-host.py').read_text()
  ...
  pathlib.Path(os.environ['RUNNER_TEMP'],'kinetra-coaching-upgrade.json').write_text(json.dumps(state,sort_keys=True)+'\n')
