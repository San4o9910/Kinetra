...
p=pathlib.Path(__file__).with_name('training-experience-upgrade-host.py');spec=importlib.util.spec_from_file_location('upgrade',p);u=importlib.util.module_from_spec(spec);spec.loader.exec_module(u)
@contextmanager
def media_owner(path):
 original=pathlib.Path.stat
 def metadata(p,*args,**kwargs):
  value=original(p,*args,**kwargs)
  if p==path:return SimpleNamespace(st_uid=1000,st_gid=1000,st_mode=value.st_mode)
  return value
 with patch.object(pathlib.Path,'stat',metadata):yield
class Checks(unittest.TestCase):
 ...
 def test_rollback_removes_only_its_own_configuration(self):
  with tempfile.TemporaryDirectory() as folder:
   p=pathlib.Path(folder)/'compose';u.write(p,b'approved');u.remove_owned_config(p,b'approved');self.assertFalse(p.exists());u.remove_owned_config(p,b'approved')
 def test_rollback_preserves_changed_configuration(self):
  with tempfile.TemporaryDirectory() as folder:
   p=pathlib.Path(folder)/'compose';u.write(p,b'changed')
   with self.assertRaisesRegex(u.Failure,'OWNED_CONFIGURATION_CHANGED'):u.remove_owned_config(p,b'approved')
 ...
 def test_existing_media_files_are_preserved(self):
  with tempfile.TemporaryDirectory() as folder:
   root=pathlib.Path(folder);media=root/'media';media.mkdir(mode=0o700)
   lesson=media/'existing.mp4';lesson.write_bytes(b'private lesson');config=root/'compose';u.write(config,b'approved')
   with media_owner(media):self.assertEqual(u.existing_media(media,config,b'approved'),media)
   self.assertEqual(lesson.read_bytes(),b'private lesson')
 def test_existing_media_config_cannot_be_silently_replaced(self):
  with tempfile.TemporaryDirectory() as folder:
   root=pathlib.Path(folder);media=root/'media';media.mkdir(mode=0o700);config=root/'compose';u.write(config,b'changed')
   with media_owner(media),self.assertRaisesRegex(u.Failure,'EXISTING_MEDIA_CONFIGURATION_CHANGED'):u.existing_media(media,config,b'approved')
   self.assertEqual(config.read_bytes(),b'changed')
 def test_shared_media_directory_is_rejected(self):
  with tempfile.TemporaryDirectory() as folder:
   root=pathlib.Path(folder);media=root/'media';media.mkdir(mode=0o755);config=root/'compose';u.write(config,b'approved')
   with media_owner(media),self.assertRaisesRegex(u.Failure,'EXISTING_MEDIA_DIRECTORY_INVALID'):u.existing_media(media,config,b'approved')
