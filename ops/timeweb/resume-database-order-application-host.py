#!/usr/bin/env python3
"""Approved continuation after database-order comparison correction.
...
# Separate continuation for the inspected September 14 stopped state.
...
RESUME_ATTEMPT = "application-database-order-attempt-" + OLD_NONCE + ".json"
RESUME_RESULT = "application-database-order-result-" + OLD_NONCE + ".json"
...
STARTED_AT = {"backend": "2026-09-14T07:09:16.50418728Z", "frontend": "2026-09-14T07:09:23.759748703Z"}
...
OWNER_APPROVAL = "cd9ac715de899a312c6ab611579a8489d6f6ec96"
OLD_ASSETS_ATTEMPT = "application-assets-attempt-" + OLD_NONCE + ".json"
OLD_ASSETS_RESULT = "application-assets-result-" + OLD_NONCE + ".json"
OLD_ASSETS_ATTEMPT_SHA = "ee37bc351f5792155d6a47ed8f011926694b27055cc1e8febe06ba40f2564486"
OLD_ASSETS_RESULT_SHA = "4fdc8a2de3957c8f0a0d369127d195484205dfb33b8fef1398a01cc3bd54ce2d"
...
def preserved_records(data):
    ...
    names = ("application-start-result.json", "application-start-attempt.json", FAILED_ARCHIVE,
             ATTEMPT_ARCHIVE, OLD_CONTINUATION_ATTEMPT, OLD_CONTINUATION_RESULT, OLD_ASSETS_ATTEMPT, OLD_ASSETS_RESULT)
    ...
    require(sha256(records[OLD_ASSETS_ATTEMPT]) == OLD_ASSETS_ATTEMPT_SHA
            and sha256(records[OLD_ASSETS_RESULT]) == OLD_ASSETS_RESULT_SHA, "PREVIOUS_ASSETS_RECORDS_CHANGED")
    require(json.loads(records[OLD_ASSETS_ATTEMPT]).get("approved") == data, "PREVIOUS_ASSETS_INPUT_CHANGED")
    assets = json.loads(records[OLD_ASSETS_RESULT])
    expected_assets = dict(expected, error="DATABASE_ISOLATION_CHANGED")
    require(all(assets.get(k) == v for k, v in expected_assets.items()), "CONFIRMED_ASSETS_ROLLBACK_REQUIRED")
    for key in ("commit", "images", "handoff_hashes", "configuration_hashes"):
        require(assets.get(key) == data[key], "FAILED_ASSETS_INPUT_CHANGED")
    return records

def validate_resume_records(data):
    require(bool(re.fullmatch(r"[a-f0-9]{40}", OWNER_APPROVAL)), "OWNER_APPROVAL_REQUIRED")
    ...
    for path in evidence.iterdir():
        require(not path.name.startswith("application-database-order-") and
                (not path.name.startswith("application-assets-") or path.name in {OLD_ASSETS_ATTEMPT, OLD_ASSETS_RESULT}) and
...
def persist_continuation_checkpoint(state, data):
    ...
    if state["result"] == "APPLICATION_LOCAL_ACCEPTED_ONLY":
        ...
        require(attempted == {"schema": 1, "owner_approval": OWNER_APPROVAL, "container_ids": PRESERVED_IDS,
                ...
                "EXACT_DURABLE_DATABASE_ORDER_ATTEMPT_REQUIRED")
        install = evidence / ("application-database-order-install-" + OLD_NONCE + ".json")
...
def main(argv=None):
    ...
    lock, data = None, None
    try:
        require(len(argv) == 3 and argv[:2] == ["--resume-database-order-application", "--private-input"], "EXPLICIT_LOCAL_ACTIVATION_REQUIRED")
