...
def main():
 ...
 api=folder=None
 try:
  assert sys.argv[1:]==['--restore-availability']
  ...
  script=(ROOT/'restore-availability-host.py').read_text()
  ...
  assert result.returncode==0 and guest['result']=='PASS_AVAILABILITY_RESTORED'
  state['guest']=guest;state['result']='PASS_AVAILABILITY_RESTORED'
  ...
  pathlib.Path(os.environ['RUNNER_TEMP'],'kinetra-availability-repair.json').write_text(json.dumps(state,sort_keys=True)+'\n')
