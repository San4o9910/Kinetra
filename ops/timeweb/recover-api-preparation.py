#!/usr/bin/env python3
"""Exact preserved-candidate recovery outer wrapper on the fixed host.
...
PINS = {
    "recover-api-candidate-host.py": "8326af893a6b25d65394841584f3d9b12b264868a592addc7c1f4cdff42128ae",
...
def main():
    ...
    try:
        ...
        child = subprocess.Popen(['/usr/bin/python3', '-B', str(folder / 'recover-api-candidate-host.py'),
            '--recover-exact-preserved-api-candidate', '--private-input', str(private_input)],
...
def provider_input(argv, environment_providers):
    if argv == ["--recover-preserved-api-environment", "--provider-env"]:
        return {key: value for key, value in environment_providers.items() if value}
    require(len(argv) == 3 and argv[:2] == ["--recover-preserved-api-environment", "--provider-input"], "EXPLICIT_API_PREPARATION_ARGUMENTS_REQUIRED")
