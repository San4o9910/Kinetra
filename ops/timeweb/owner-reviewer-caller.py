...
def main():
 ...
 api=folder=None
 try:
  assert sys.argv[1:]==['--grant-owner-reviewer']
  ...
  script=(ROOT/'owner-reviewer-grant-host.py').read_text()
  result=subprocess.run(args+['/usr/bin/python3 -B -c '+shlex.quote(script)],input=json.dumps({'run':os.environ['GITHUB_RUN_ID']}).encode(),stdout=subprocess.PIPE,stderr=subprocess.DEVNULL,timeout=240)
  assert len(result.stdout)<65536
  guest=json.loads(result.stdout);state['guest']=guest
  assert result.returncode==0 and guest['result']=='PASS_REVIEWER_ACCESS'
  state['result']='PASS_REVIEWER_ACCESS'
  ...
  pathlib.Path(os.environ['RUNNER_TEMP'],'kinetra-owner-reviewer.json').write_text(json.dumps(state,sort_keys=True)+'\n')
 return 0 if state['result']=='PASS_REVIEWER_ACCESS' else 1
