...
def main():
 ...
 api=folder=None
 try:
  assert sys.argv[1:]==['--upgrade-trainer-workspace']
  ...
  request=pathlib.Path(os.environ['RUNNER_TEMP'],'kinetra-trainer-workspace-request.json')
  ...
  script=(ROOT/'trainer-workspace-upgrade-host.py').read_text()
  ...
  pathlib.Path(os.environ['RUNNER_TEMP'],'kinetra-trainer-workspace-upgrade.json').write_text(json.dumps(state,sort_keys=True)+'\n')
