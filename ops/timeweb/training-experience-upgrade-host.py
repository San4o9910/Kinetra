"""Install personal trainer programs, additive marketplace migration 020 and private host media.
...
S=pathlib.Path('/srv/kinetra-stage');APP='4a4560fac325dc5a09f8ca124ea6101072558b72';BASE='125af45560e86fbe5f47837ffabe813e4aebfb4d'
OLD_IMAGES={'backend':'ghcr.io/san4o9910/kinetra-backend@sha256:856fcf6b8a0ae5ac7416e310ca18c7b3befb8be03f47886247ffeedd04b2af55','frontend':'ghcr.io/san4o9910/kinetra-frontend@sha256:5620c17d13faac021164f860bb69ccb1001997f5489a34d1cd50575d4c78cb4f'}
...
assert.equal(filename,'020_marketplace_catalogue.sql');await c.query('BEGIN');try{await c.query(text);await c.query('INSERT INTO schema_migrations(filename,checksum)VALUES($1,$2)',[filename,checksum]);await c.query('COMMIT')}catch(e){await c.query('ROLLBACK');throw e}}
...
def main():
 ...
 try:
  ...
  oldledger=ledger(pgid);expected=data['migrations'];require(len(expected)==20 and len(oldledger)==19 and all(expected.get(r['filename'])==r['checksum'] for r in oldledger),'MIGRATION_HISTORY_CHANGED')
  ...
  prior=S/'nutrition-coach-release-125af45560e8-35434465114'
  ...
  work=S/('marketplace-release-'+APP[:12]+'-'+data['run']);work.mkdir(mode=0o700)
  ...
  media_check="""import assert from 'node:assert/strict';import {writeFile,unlink,stat} from 'node:fs/promises';import {randomUUID} from 'node:crypto';import {databasePool} from './apps/backend/dist/db/pool.js';import {TrainingService} from './apps/backend/dist/training/service.js';import {TrainingExperience} from './apps/backend/dist/training/experience.js';import {NutritionService} from './apps/backend/dist/training/nutrition.js';import {CoachProfiles} from './apps/backend/dist/training/coach-profile.js';import {MarketplaceService} from './apps/backend/dist/marketplace/service.js';import {env} from './apps/backend/dist/config/env.js';
...
const service=new TrainingService(databasePool,true);assert.deepEqual(await service.myTraining(randomUUID()),{trainer_name:null,student_id:null,plans:[],assigned_lessons:[]});const experience=new TrainingExperience(service);assert.deepEqual(await experience.measurements(randomUUID()),{measurements:[]});assert.deepEqual(await experience.reschedules(randomUUID()),{requests:[]});assert.deepEqual(await experience.complaints(randomUUID()),{complaints:[]});const nutrition=new NutritionService(service);assert.deepEqual(await nutrition.list(randomUUID(),'2026-09-19'),{entries:[]});assert.deepEqual(await nutrition.templates(randomUUID()),{templates:[]});assert.deepEqual(await new CoachProfiles(service).mine(randomUUID()),{profile:null,score:null,can_review:false});assert.equal(process.env.TRAINING_REMINDERS_ENABLED,'true');const market=await new MarketplaceService(service).catalogue({});assert.equal(market.purchases_enabled,false);assert.ok(Array.isArray(market.trainers)&&Array.isArray(market.offers));await databasePool.query('SELECT id FROM marketplace_publication_events LIMIT 0');await databasePool.end();console.log('LIVE_TRAINER_WORKSPACE=PASS');"""
  ...
  require(all(get(p)[0]==401 for p in ('/api/v1/training/marketplace/workspace','/api/v1/training/marketplace/review','/api/v1/training/students','/api/v1/training/mine','/api/v1/training/lessons','/api/v1/training/templates','/api/v1/training/attention','/api/v1/training/nutrition?date=2026-09-19','/api/v1/training/nutrition-templates','/api/v1/training/coach-profile','/api/v1/training/my-coach','/api/v1/training/measurements','/api/v1/training/reschedules','/api/v1/training/admin/complaints','/api/v1/training/lessons/00000000-0000-4000-8000-000000000001/assignments','/api/v1/training/media/00000000-0000-4000-8000-000000000001?token=invalid')),'TRAINING_AUTH_BOUNDARY_FAILED')
  code,body=get('/api/v1/marketplace/');require(code==200 and json.loads(body)['purchases_enabled'] is False,'PUBLIC_MARKETPLACE_FAILED')
