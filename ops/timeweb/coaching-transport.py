...
def main():
 ...
 api=folder=None
 try:
  assert sys.argv[1:]==['--inspect-coaching-release']
  ...
  script=(ROOT/'coaching-host-inspect.py').read_text()
  ...
  pathlib.Path(os.environ['RUNNER_TEMP'],'kinetra-coaching-inspection.json').write_text(json.dumps(state,sort_keys=True)+'\n')
